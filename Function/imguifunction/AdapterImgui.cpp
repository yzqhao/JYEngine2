...
void AdapterImgui::LoadImgui(sol::state& sol_state)
{
	Bind_Imgui(sol_state);
}

void AdapterImgui::_OnCreate()
{
	Engine::Instance()->AddEarlyUpdateCallback(DelegateEvent<void()>::Handler::FromMethod<AdapterImgui, &AdapterImgui::EarlyUpdate>(this));
	Engine::Instance()->AddLateUpdateCallback(DelegateEvent<void()>::Handler::FromMethod<AdapterImgui, &AdapterImgui::LateUpdate>(this));
