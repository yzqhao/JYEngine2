...
const FColor FColor::Gray(127, 127, 127);
...
FColor FLinearColor::ToRGBE() const
{
	...
	else
	{
		...
		Color.R = (byte)Clamp(int(R * Scale), 0, 255);
		Color.G = (byte)Clamp(int(G * Scale), 0, 255);
		Color.B = (byte)Clamp(int(B * Scale), 0, 255);
		Color.A = (byte)(Clamp(Exponent, -128, 127) + 128);
	}
...
FColor FLinearColor::ToFColor(const bool bSRGB) const
{
	...
	Result.A = (byte)floor(FloatA * 255.999f);
	Result.R = (byte)floor(FloatR * 255.999f);
	Result.G = (byte)floor(FloatG * 255.999f);
	Result.B = (byte)floor(FloatB * 255.999f);
...
{
	return FColor(
		(byte)Clamp<int>(int(R * 255.f), 0, 255),
		(byte)Clamp<int>(int(G * 255.f), 0, 255),
		(byte)Clamp<int>(int(B * 255.f), 0, 255),
		(byte)Clamp<int>(int(A * 255.f), 0, 255)
	);
}
...
{
	return FColor(
		(byte)Clamp<int>(RoundToInt(R * 255.f), 0, 255),
		(byte)Clamp<int>(RoundToInt(G * 255.f), 0, 255),
		(byte)Clamp<int>(RoundToInt(B * 255.f), 0, 255),
		(byte)Clamp<int>(RoundToInt(A * 255.f), 0, 255)
	);
}
 ...
 */
FLinearColor FLinearColor::MakeFromHSV8(byte H, byte S, byte V)
{
...
{
	const byte Hue = (byte)(FRand() * 255.f);
...
{
	const float RedSclr = Clamp<float>((1.0f - Scalar) / 0.5f, 0.f, 1.f);
	const float GreenSclr = Clamp<float>((Scalar / 0.5f), 0.f, 1.f);
	const byte R = (byte)int(255 * RedSclr);
	const byte G = (byte)int(255 * GreenSclr);
	const byte B = 0;
	return FColor(R, G, B);
}
...
{
	float MaxComponent = Max(M_EPSILON, Max(InLinearColor.R, Max(InLinearColor.G, InLinearColor.B)));
