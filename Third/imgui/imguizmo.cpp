...
namespace ImGui
{
	...
	struct Context
	{
		...
		vec_t mMatrixOrigin;
		bool  mEndTrans; //translation moving end signal  (for faceimage2d)
		...
		// face liquify circle using
		bool mbEllipseMoving;
	...
	static const char* scaleInfoMask[] = {"X : %5.2f", "Y : %5.2f", "Z : %5.2f", "XYZ : %5.2f", "XY : %5.2f"};
	...
	static int GetRotateType(bool show2D = false);
	static int GetScaleType(bool show2D = false);
	...
	bool is_using() //从Ethereal引擎测试输出值来看  该函数表示当前物体坐标轴是否被选中ing
	{
	...
	void enable(bool enable)
	{
		...
		if(!enable)
		{
			...
			gContext.mbEllipseMoving = false;
		}
	}
		...
		if(gContext.mbEnable)
		{
			switch(op)
			{
				...
				case rotate:
					...
					break;
				case scale:
					colors[0] = (type == SCALE_XYZ) ? selectionColor : 0xFFFFFF50;
					for(int i = 0; i < 3; i++)
	...
	static void DrawRotationGizmo(int type, bool showXY = false)
	{
	...
	static void DrawScaleGizmo(int type, bool show2D = false)
	{
		ImDrawList* drawList = gContext.mDrawList;
		...
		{
			if (show2D && i > 1)//Avoid Drawing Z axis
				continue;
		...
		if(gContext.mbUsing)
		{
			...
			*/
			...
			bool movingXY = type == SCALE_XYZ && show2D;
			ImFormatString(tmps, sizeof(tmps), movingXY ? scaleInfoMask[type - SCALE_X + 1] : scaleInfoMask[type - SCALE_X],
	...
	static void DrawTranslationGizmo(int type, bool show2D = false)
	{
		ImDrawList* drawList = gContext.mDrawList;
		if(!drawList)
			return;
		...
		for(unsigned int i = 0; i < coordNum; ++i)
		{
			...
			if (show2D && i == 1)//Avoid drawing YZ plane
				continue;
		...
		if(!show2D) //Avoid drawing model origin point(Z value will be changed)
			drawList->AddCircleFilled(gContext.mScreenSquareCenter, 6.f, colors[0], 32);

		if(gContext.mbUsing)//正在拖拽
		{
			ImVec2 sourcePosOnScreen = worldToPos(gContext.mMatrixOrigin, gContext.mViewProjection);
			ImVec2 destinationPosOnScreen = worldToPos(gContext.mModel.v.position, gContext.mViewProjection);
	...
	static int GetScaleType(bool show2D)
	{
		ImGuiIO& io = ImGui::GetIO();
		int type = NONE;
		...
		if (show2D && type == SCALE_Z)
			type = NONE;
		return type;
	}

	static int GetRotateType(bool show2D)
	{
		ImGuiIO& io = ImGui::GetIO();
		int type = NONE;
		...
		if(dist >= (gContext.mRadiusSquareCenter - 8.0f) && dist < (gContext.mRadiusSquareCenter + 8.0f))
			type = ROTATE_SCREEN;
		...
		for(unsigned int i = 0; i < 3 && type == NONE; i++)
		{
			...
			if(distance > 0.75f && distance < 1.35f)
				type = ROTATE_X + i;
		}
		
		if (show2D && type != ROTATE_SCREEN)//2D scene Only use ROTATE_SCREEN
			type = NONE;
	...
	//理解成要么正在拖动辅助体(gContext.mbUsing)  要么正在寻找辅助体
	static void HandleTranslation(float* matrix, float* deltaMatrix, int& type, float* snap, bool show2D = false)
	{
		ImGuiIO& io = ImGui::GetIO();
		...
		if(gContext.mbUsing)
		{
			...
			*(matrix_t*)matrix = res;
			...
			if (!io.MouseDown[0])
			{
				gContext.mEndTrans = true;
				gContext.mbUsing = false;
				//std::cout << "gContext.mbUsing" << std::endl;
			}
		...
		else
		{
			...
			if (show2D && (type > 4 || type == 3))//Avoid  Changing Z Value when running in 2D scene
			{
				type = NONE;
				gContext.mbUsing = false;//1738行设置为true了 需要false掉... 旋转和缩放有NONE判断
			}
			//std::cout << "not moving " << std::endl;
			gContext.mEndTrans = false;
		}
		
	}

	static void HandleScale(float* matrix, float* deltaMatrix, int& type, float* snap, bool show2D = false)
	{
		ImGuiIO& io = ImGui::GetIO();
		...
		if(gContext.mbUsing)
		{
			...
			if(gContext.mCurrentOperation >= SCALE_X && gContext.mCurrentOperation <= SCALE_Z)
			{
				...
				const vec_t& axisValue = *(vec_t*)&gContext.mModel.m[axisIndex];//current axis direct
				vec_t ndcCoord = axisValue;
				ndcCoord.Transform(gContext.mViewProjection);//project into screen
				//std::cout << "ndcCoord is " << ndcCoord.x << " " << ndcCoord.y << " " << ndcCoord.z << " " << ndcCoord.w << " " << std::endl;
				vec_t dragVec;
				dragVec.x = io.MousePos.x - gContext.mSaveMousePosx;
				dragVec.y = gContext.mSaveMousePosy - io.MousePos.y;//imgui-y towards down
				float direct = (dragVec.x * ndcCoord.x + dragVec.y * ndcCoord.y) > 0 ? 1.0f : -1.0f;
				//std::cout << "direct " << direct << std::endl;
				dragVec.z = 0.0f;
				dragVec.w = 0.0f;
 				float scaleDelta = dragVec.Length() * 0.01f * direct;
				gContext.mScale[axisIndex] = max(1.f + scaleDelta, 0.01f);
			}
			else
			{
			...
			//2D  Z not allow scaled
			if (show2D)
				gContext.mScale[2] = 1.0f;
	...
	static void HandleRotation(float* matrix, float* deltaMatrix, int& type, float* snap, bool show2D = false)
	{
		ImGuiIO& io = ImGui::GetIO();
	...
	void manipulate(const float* view, const float* projection, operation op, mode mod, float* matrix,
					float* deltaMatrix, float* snap, float* localBounds, float* boundsSnap, bool show2D)
	{
		set_draw_list();
		...
		if(gContext.mbEnable)
		{
			if(!gContext.mbUsingBounds)
			{
				...
				{
					case rotate:
						HandleRotation(matrix, deltaMatrix, type, snap, show2D);
						break;
					case translate:
						HandleTranslation(matrix, deltaMatrix, type, snap, show2D);
						break;
					case scale:
						HandleScale(matrix, deltaMatrix, type, snap, show2D);
						break;
				}
			}
		}
		...
		if(!gContext.mbUsingBounds)
		{
			...
			{
				case rotate:
					DrawRotationGizmo(type, show2D);
					break;
				case translate:
					DrawTranslationGizmo(type, show2D);
					break;
				case scale:
					DrawScaleGizmo(type, show2D);
					break;
			}
		}
	}
	...
	void draw_ellipse(float* position, float rx, float ry, float r, float g, float b, float a, float* px, float* py, bool is_dashed)
	{
		...
		float unitRadius = (worldToPos(makeVect(1.0f, 0), gContext.mViewProjection) - worldToPos(makeVect(0, 0), gContext.mViewProjection)).x;
		float radiusX = rx * unitRadius * 0.1;
		float radiusY = ry * unitRadius * 0.1;

		ImVec2 pixel_pos = worldToPos(makeVect(position[0], position[1]), gContext.mViewProjection);

		*px = pixel_pos.x;
		*py = pixel_pos.y;

		float delta_x = (pixel_pos.x - io.MousePos.x) / radiusX;
		float delta_y = (pixel_pos.y - io.MousePos.y) / radiusY;
		float is_hovered = (delta_x * delta_x + delta_y * delta_y) <= 1;

		if (is_hovered)  a /= 2.0;
		ImU32 color = ImColor(ImVec4(r, g, b, a));

		if (is_dashed) {
			drawList->AddDashedEllipse(pixel_pos, radiusX, radiusY, 1, color, 32, unitRadius * 0.002);
		}
		else {
			drawList->AddEllipse(pixel_pos, radiusX, radiusY, 1, color, 32, unitRadius * 0.002);
		}

		if (gContext.mbEnable && is_hovered && CanActivate() && !gContext.mbEllipseMoving)
		{
			gContext.mbEllipseMoving = true;
		}
		if (gContext.mbEllipseMoving)
		{
			position[0] += io.MouseDelta.x / unitRadius;
			position[1] -= io.MouseDelta.y / unitRadius;
		}
		if (!io.MouseDown[0])
		{
			gContext.mbEllipseMoving = false;
