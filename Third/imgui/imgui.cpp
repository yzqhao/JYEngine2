...
//imgui-node-editor蓝图用到的imgui扩展函数
// Stack Layout
...
void ImGui::RenderCheckMark(ImVec2 pos, ImU32 col, float sz, float thick)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;

    float thickness = thick == 0 ? ImMax(sz / 5.0f, 1.0f): thick;
    sz -= thickness*0.5f;
...
bool ImGui::IsMouseRightDown()
{
	return GImGui->IO.MouseDown[1];
}

bool ImGui::IsMouseLeftDown()
{
	return GImGui->IO.MouseDown[0];
}
...
ImVec4 ImGui::GetContentsRegionRectbyName(const char* name)
{
	ImVec4 vec4;
	ImGuiWindow* window = FindWindowByName(name);
	//ImRect r1 = window->WorkRect;
	ImRect r2 = window->ContentsRegionRect;
	if (window)
	{
		vec4.x = r2.Min.x;
		vec4.y = r2.Min.y;
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode* node, ImGuiWindow* host_window)
{
    ...
    for (int window_n = 0; window_n < node->Windows.Size; window_n++)
    {
        ...
        if (window->LastFrameActive + 1 >= g.FrameCount || !node_was_active)
        {
            ...
            bool tab_open = true;
            TabItemEx(tab_bar, window->Name, nullptr, tab_item_flags, window);
            if (!tab_open)
...
static void ImGui::DockNodePreviewDockCalc(ImGuiWindow* host_window, ImGuiDockNode* host_node, ImGuiWindow* root_payload, ImGuiDockPreviewData* data, bool is_explicit_target, bool is_outer_docking)
{
    ...
    data->FutureNode.HasCloseButton = false;
