...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg)
{
	...
	//FIXME(hjh) 窗口没有固定住蓝图创建treenode(状态机要用这个uinode)  拖拽创建新treenode连接会崩溃.. Mark一个
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    window->DrawList->AddImage(user_texture_id, image_bb.Min + ImVec2(1, 1), image_bb.Max - ImVec2(1, 1), uv0, uv1, GetColorU32(tint_col));
...
bool ImGui::TreeNodeEx(const char* label, ImGuiTreeNodeFlags flags, ImTextureID id,
                       ImVec2 size, bool checkable, bool cur_checked, bool sub_checked, bool* output_checked)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;

	return TreeNodeBehavior(window->GetID(label), flags, label, nullptr, id, size, checkable, cur_checked, sub_checked, output_checked);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end,
                             ImTextureID image, ImVec2 size, bool checkable, bool cur_checked, bool sub_checked, bool* output_checked)

{
    ...
    const float text_offset_x = (g.FontSize + (display_frame ? padding.x*2 : padding.x));   // Collapser arrow width + Spacing
    ...
    // Render
    ImU32 bg_col = GetColorU32((held && hovered) || selected ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
    const ImU32 text_col = GetColorU32(ImGuiCol_Text);
    
    const auto add_check_box = [&]() -> bool
    {
        const auto start = ImVec2(frame_bb.Max.x - 32, frame_bb.Min.y + frame_bb.GetHeight() / 2 - 8);
        window->DrawList->AddRect(start, start + ImVec2(16, 16), GetColorU32(ImGuiCol_Text), 0, 0, 1.0f);

        if (cur_checked)
        {
            const float pad = ImMax(1.0f, (float)(int)(16 / 6.0f));
            RenderCheckMark(start + ImVec2(pad, pad), GetColorU32(ImGuiCol_Text), 16 - pad * 2.0f, 1.0);
        }

        SameLine();
    	
        const auto& bb = ImRect(start, start + ImVec2(16, 16));
        const auto& checkboxId = window->GetID((std::string(label) + "#checkbox").c_str());
        ItemSize(size);
        return ButtonBehavior(bb, checkboxId, nullptr, nullptr) ? !cur_checked : cur_checked;
    };

    const ImVec2 image_pos = frame_bb.Min + ImVec2(text_offset_x, (frame_bb.GetHeight() - size.y) / 2);
    const ImVec2 text_pos = frame_bb.Min + ImVec2(text_offset_x, text_base_offset_y) + ImVec2(size.x + padding.x, 0);
    const ImVec2 select_start = ImVec2(window->WorkRect.GetTL().x, frame_bb.Min.y);
	
    ImGuiNavHighlightFlags nav_highlight_flags = ImGuiNavHighlightFlags_TypeThin;
    if (display_frame)
    {
        // Framed type
        RenderFrame(select_start, frame_bb.Max, bg_col, true, style.FrameRounding);
        ...
        if (image)
        {
            window->DrawList->AddImage(image, image_pos, image_pos + size, ImVec2(0, 0), ImVec2(1, 1), text_col);
        }
        ...
        if (checkable)
        {
            *output_checked = add_check_box();
        }
    }
    else
    {
        // Unframed typed for tree nodes
        if (hovered || selected || sub_checked)
        {
            RenderFrame(select_start, frame_bb.Max, bg_col, false);
            RenderNavHighlight(frame_bb, id, nav_highlight_flags);
        }
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, frame_bb.Min + ImVec2(padding.x, g.FontSize*0.15f + text_base_offset_y), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.8f);
        if (g.LogEnabled)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        tab->WidthContents = TabItemCalcSize(tab_name, false).x;
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
    ...
    if (close_button_id != 0)
        if (g.HoveredId == tab_id || g.HoveredId == close_button_id || g.ActiveId == close_button_id)
            close_button_visible = false; // remove close button
