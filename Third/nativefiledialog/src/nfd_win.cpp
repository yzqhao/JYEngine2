...
static nfdresult_t SetDefaultFileName(IFileDialog *dialog, const nfdchar_t *defaultName)
{
	if (!defaultName || strlen(defaultName) == 0)
		return NFD_OKAY;

	wchar_t *defaultNameW = { 0 };
	CopyNFDCharToWChar(defaultName, &defaultNameW);

	HRESULT result = dialog->SetFileName(defaultNameW);

	if (!SUCCEEDED(result))
	{
		NFDi_SetError("Error set default file name");
		NFDi_Free(defaultNameW);
		return NFD_ERROR;
	}
...
static nfdresult_t SetDefaultExtension(IFileDialog *dialog, const nfdchar_t *defaultExtension)
{
	if (!defaultExtension || strlen(defaultExtension) == 0)
		return NFD_OKAY;

	wchar_t *defaultExtensionW = { 0 };
	CopyNFDCharToWChar(defaultExtension, &defaultExtensionW);

	HRESULT result = dialog->SetDefaultExtension(defaultExtensionW);

	if (!SUCCEEDED(result))
	{
		NFDi_SetError("Error set default extension");
		NFDi_Free(defaultExtensionW);
		return NFD_ERROR;
	}
                            ...
                            nfdchar_t **outPath )
{
	...
	if (!SetDefaultFileName(fileSaveDialog, defaultName))
	{
		goto end;
	}

	if (!SetDefaultExtension(fileSaveDialog, filterList))
	{
		goto end;
	}
    // Show the dialog.
