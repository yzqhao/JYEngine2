...
#	include "OpenGL/glimports.h"
	...
	struct SwapChainGL
	{
			...
			, m_depthStencilRbo(0)
		{
            if(_layer) {
                ...
                //外部创建 需要保证fbo 和 rbo的状态
                glGetIntegerv(GL_FRAMEBUFFER_BINDING, (GLint*)&m_fbo);
                glGetIntegerv(GL_RENDERBUFFER_BINDING, (GLint*)&m_colorRbo);
            }
	...
	{
		if (m_needFree && 0 != m_fbo)
		{
			GL_CHECK(glDeleteFramebuffers(1, &m_fbo) );
		}

		if (m_needFree && 0 != m_colorRbo)
		{
			GL_CHECK(glDeleteRenderbuffers(1, &m_colorRbo) );
		}

		if (m_needFree && 0 != m_depthStencilRbo)
		{
			GL_CHECK(glDeleteRenderbuffers(1, &m_depthStencilRbo) );
		}
