		...
		if (m_id != 0)
		{
			...
			if (m_type != GL_COMPUTE_SHADER)
			{
				...
				if ((!isCompleteShader) && (isAndroid || isIOS || isDesktop))
				{
					...
					bool bEs30 = false;	// (OpenGL::GetCurrentFeatureLevel() == DeviceFeatureLevel::ES3_0);
					bool bEs20 = false;	// (OpenGL::GetCurrentFeatureLevel() == DeviceFeatureLevel::ES2);
					...
					bool bFramebufferFetch = false;// OpenGL::NeedFrameBufferFetchDef();
					...
					{
						bool bMRTFramebufferFetch = false;	//OpenGL::SupportsShaderMRTFramebufferFetch()
						bool bMRTFramebufferFetchDS = false;	// OpenGL::SupportFrameBufferFetchDepthStencil();
...

					{
						...
						else if (isAndroid)
						{
							...
							// need gl externs
							bool supportTexLod = false;	// OpenGL::SupportsShaderTextureLod();
