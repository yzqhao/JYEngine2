...
#undef CONVERT_HANDLE
	...
	inline bool isCustomShader(uint32_t _magic)
	{
		return BX_MAKEFOURCC(0, 0, 'F', 0) == (_magic & BX_MAKEFOURCC(0, 0, 0xff, 0));
	}

	inline bool isShaderBin(uint32_t _magic)
	{
		bool binMgic = BX_MAKEFOURCC(0, 'S', 'H', 0) == (_magic & BX_MAKEFOURCC(0, 0xff, 0xff, 0));
		binMgic = binMgic || BX_MAKEFOURCC(0, 'S', 'F', 0) == (_magic & BX_MAKEFOURCC(0, 0xff, 0xff, 0));
		binMgic = binMgic || BX_MAKEFOURCC(0, 'S', 'G', 0) == (_magic & BX_MAKEFOURCC(0, 0xff, 0xff, 0));
		binMgic = binMgic && (isShaderType(_magic, 'C') || isShaderType(_magic, 'F') || isShaderType(_magic, 'V'));
		return binMgic;
	}
	...
	constexpr uint64_t kInValidProgram = uint64_t(1) << kSortKeyDrawTypeBitShift;
	...
	struct SortKey
	{
		uint64_t encodeDraw()
		{
			ProgramHandle progrHandle;
			progrHandle.idx = m_program.idx;
			if (bgfx::isValid(progrHandle))
			{
				...
				const uint64_t key = view | kSortKeyDrawBit | kInValidProgram | program;

				return key;
			}
			else
			{
				const uint64_t program = (uint64_t(0) << kSortKeyDraw2ProgramShift) & kSortKeyDraw2ProgramMask;
				const uint64_t view = (uint64_t(m_view) << kSortKeyViewBitShift) & kSortKeyViewMask;
				const uint64_t key = view | kSortKeyDrawBit | program;
		...
		uint64_t encodeCompute()
		{
			...
			const uint64_t view = (uint64_t(m_view) << kSortKeyViewBitShift) & kSortKeyViewMask;
			const uint64_t key = program | view;
		...
		{
			m_view = (_key & kSortKeyViewMask) >> kSortKeyViewBitShift;
			if (_key & kSortKeyDrawBit)
			{
				uint64_t validProgram = _key & kInValidProgram;
...
#undef SORT_KEY_RENDER_DRAW
	...
	public:
		...
		const UniformRegInfo* find(const char* _name, UniformType::Enum uniformType) const
		{
			const uint32_t key = bx::hash<bx::HashMurmur2A>(_name) ^ (uint32_t)(uniformType);
			uint16_t handle = m_uniforms.find(key);
		...
		const UniformRegInfo& add(UniformHandle _handle, const char* _name, UniformType::Enum uniformType)
		{
	...
	struct BX_NO_VTABLE RendererContextI
	{
		...
		virtual DeviceFeatureLevel::Enum getDeviceFeatureLevel() const = 0;
		...
		virtual void resizeTexture(TextureHandle _handle, uint16_t _width, uint16_t _height, uint8_t _numMips, uint16_t _numLayers, bool autoGenMipmap) = 0;
		...
		virtual void createFrameBuffer(FrameBufferHandle _handle, int32_t _refHandle, uint32_t _width, uint32_t _height, TextureFormat::Enum _format, TextureFormat::Enum _depthFormat) = 0;
		...
		virtual void submit(Frame* _render, TextVideoMemBlitter& _textVideoMemBlitter) = 0;
...
#	define BGFX_API_FUNC(_func) _func
	...
	struct Context
	{
		...
		void resizeTexture(TextureHandle _handle, uint16_t _width, uint16_t _height, uint8_t _numMips, uint16_t _numLayers, bool autoGenMipmap)
		{
			const TextureRef& ref = m_textureRef[_handle.idx];
		...
		BGFX_API_FUNC(FrameBufferHandle createFrameBuffer(int32_t refHandle, uint16_t _width, uint16_t _height, TextureFormat::Enum _format, TextureFormat::Enum _depthFormat))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
			...
			if (isValid(handle))
			{
				CommandBuffer& cmdbuf = getCommandBuffer(CommandBuffer::CreateFrameBuffer);
				cmdbuf.write(handle);
				cmdbuf.write((int32_t)2);

				cmdbuf.write(refHandle);
				cmdbuf.write(_width);
				cmdbuf.write(_height);
				cmdbuf.write(_format);
				cmdbuf.write(_depthFormat);
...
//			setName(convert(_handle), _name);
		...
		BGFX_API_FUNC(UniformHandle createUniform(const char* _name, UniformType::Enum _type, uint16_t _num) )
		{
			...
			uint32_t hashV = bx::hash<bx::HashMurmur2A>(_name) ^ (uint32_t)(_type);
			uint16_t idx = m_uniformHashMap.find(hashV);
			if (kInvalidHandle != idx)
			{
			...
			bool ok = m_uniformHashMap.insert(hashV, handle.idx);
