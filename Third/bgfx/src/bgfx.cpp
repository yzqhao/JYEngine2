...
#define BGFX_CHECK_CAPS(_caps, _msg)                                                   \
	...
	{
		this->supported = 0;
		this->featurelevel = DeviceFeatureLevel::NONE;
	}
...
#undef LIMITS
	...
	bool Context::init(const Init& _init)
	{
		...
		g_caps.featurelevel = m_renderCtx->getDeviceFeatureLevel();
			...
			{
				{
					...
					m_renderCtx->submit(m_render, m_textVideoMemBlitter);
					m_flipped = false;
				}
			...
			case CommandBuffer::ResizeTexture:
				{
					...
					bool autoGenMipmap;
					_cmdbuf.read(autoGenMipmap);

					m_renderCtx->resizeTexture(handle, width, height, numMips, numLayers, autoGenMipmap);
				}
				break;
			...
			case CommandBuffer::CreateFrameBuffer:
				{
					...
					else if (tag == 0)  // tag = 0 为attach的rt
					{
						uint8_t num;
						_cmdbuf.read(num);
					...
					else if (tag == 2) // tag = 2 为ref的rt
					{
						int32_t refHandle;
						_cmdbuf.read(refHandle);
						...
						m_renderCtx->createFrameBuffer(handle, refHandle, width, height, format, depthFormat);
					}
				}
				break;
...
#define BGFX_ENCODER(_func) reinterpret_cast<EncoderImpl*>(this)->_func
	...
	void Encoder::setCurrentViewID(uint16_t viewID)
	{
		BGFX_ENCODER(setCurrentViewID(viewID));
	}

	uint16_t Encoder::getCurrentViewID()
	{
		return BGFX_ENCODER(getCurrentViewID());
	}
...
#undef BGFX_ENCODER
	...
	DeviceFeatureLevel::Enum getDeviceFeatureLevel()
	{
		return g_caps.featurelevel;
	}
	...
	void calcTextureSize(TextureInfo& _info, uint16_t _width, uint16_t _height, uint16_t _depth, bool _cubeMap, bool _hasMips, uint16_t _numLayers,
		TextureFormat::Enum _format, bool _autoGenMipmap, int32_t refHandle)
	{
		bimg::imageGetSize( (bimg::TextureInfo*)&_info, _width, _height, _depth, _cubeMap, _hasMips, _numLayers, bimg::TextureFormat::Enum(_format), _autoGenMipmap, -1);
	}
	...
	static TextureHandle createTexture2D(BackbufferRatio::Enum _ratio, uint16_t _width, uint16_t _height, bool _hasMips, uint16_t _numLayers, TextureFormat::Enum _format, uint64_t _flags,
		const Memory* _mem, bool autoGenMipMap, int32_t refHandle, int32_t mipNums)
	{
		...
		const uint8_t numMips = (mipNums == 0 ? calcNumMips(_hasMips, _width, _height) : mipNums);
		...
		{
			TextureInfo ti;
			calcTextureSize(ti, _width, _height, 1, false, _hasMips, _numLayers, _format, autoGenMipMap, refHandle);
	...
	TextureHandle createTexture2D(uint16_t _width, uint16_t _height, bool _hasMips, uint16_t _numLayers
		, TextureFormat::Enum _format, uint64_t _flags, const Memory* _mem, bool m_autoGenMipmap, int32_t refHandle, int32_t mipNums)
	{
		BX_ASSERT(_width > 0 && _height > 0, "Invalid texture size (width %d, height %d).", _width, _height);
		return createTexture2D(BackbufferRatio::Count, _width, _height, _hasMips, _numLayers, _format, _flags, _mem, m_autoGenMipmap, refHandle, mipNums);
	}

	TextureHandle createTexture2D(BackbufferRatio::Enum _ratio, bool _hasMips, uint16_t _numLayers,
		TextureFormat::Enum _format, uint64_t _flags, bool m_autoGenMipmap, int32_t refHandle, int32_t mipNums)
	{
		BX_ASSERT(_ratio < BackbufferRatio::Count, "Invalid back buffer ratio.");
		return createTexture2D(_ratio, 0, 0, _hasMips, _numLayers, _format, _flags, NULL, m_autoGenMipmap, refHandle, mipNums);
	}
	...
	TextureHandle createTexture3D(uint16_t _width, uint16_t _height, uint16_t _depth, bool _hasMips, TextureFormat::Enum _format, uint64_t _flags, const Memory* _mem)
	{
		...
		{
			TextureInfo ti;
			calcTextureSize(ti, _width, _height, _depth, false, _hasMips, 1, _format, false, -1);
			BX_ASSERT(ti.storageSize == _mem->size
		...
		tc.m_cubeMap   = false;
		tc.m_mem = _mem;
		tc.m_autoGenMipmap = false;
		bx::write(&writer, tc, bx::ErrorAssert{});

		return s_ctx->createTexture(mem, _flags, 0, NULL, BackbufferRatio::Count, NULL != _mem);
	}

	TextureHandle createTextureCube(uint16_t _size, bool _hasMips, uint16_t _numLayers, TextureFormat::Enum _format, uint64_t _flags, const Memory* _mem, bool autoGenMipmap, int32_t refHandle, int32_t mipNums)
	{
		bx::ErrorAssert err;
		...
		const uint8_t numMips = (mipNums == 0 ? calcNumMips(_hasMips, _size, _size) : mipNums);
		_numLayers = bx::max<uint16_t>(_numLayers, 1);
		...
		{
			TextureInfo ti;
			calcTextureSize(ti, _size, _size, 1, true, _hasMips, _numLayers, _format, false, -1);
			BX_ASSERT(ti.storageSize == _mem->size
	...
	FrameBufferHandle createFrameBuffer(int32_t refHandle, uint16_t _width, uint16_t _height, TextureFormat::Enum _format, TextureFormat::Enum _depthFormat)
	{
		BGFX_CHECK_CAPS(BGFX_CAPS_SWAP_CHAIN, "Swap chain is not supported!");
		BX_WARN(_width > 0 && _height > 0
			, "Invalid frame buffer dimensions (width %d, height %d)."
	...
	void setCurrentViewID(uint16_t viewID)
	{
		BGFX_CHECK_API_THREAD();
		s_ctx->m_encoder0->setCurrentViewID(viewID);
	}

	uint16_t getCurrentViewID()
	{
		BGFX_CHECK_API_THREAD();
		return s_ctx->m_encoder0->getCurrentViewID();
	}
