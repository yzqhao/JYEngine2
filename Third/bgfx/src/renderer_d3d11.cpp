...
//						| D3D11_CREATE_DEVICE_PREVENT_INTERNAL_THREADING_OPTIMIZATIONS
...

			{
				...
				g_caps.supported2 = (g_caps.supported2 | BGFX_CAPS_SHADOW_SAMPLER);
				g_caps.supported2 |= BGFX_CAPS_FRAMEBUFFER_HALF;
				g_caps.supported2 |= BGFX_CAPS_FRAMEBUFFER_FLOAT;
				g_caps.supported2 |= BGFX_CAPS_SRGB;
		...
		void createFrameBuffer(FrameBufferHandle _handle, int32_t _refHandle, uint32_t _width, uint32_t _height, TextureFormat::Enum _format, TextureFormat::Enum _depthFormat) override
		{
			//暂时不支持通过外面设置handle的方式来使用
		}
		...
		void blitSetup(TextVideoMemBlitter& _blitter) override
		{
			...
			setShaderUniform(flags, predefined.m_loc, proj, 64);
...
//			invalidateCache();
		...
		void setShaderUniform(uint8_t _flags, uint32_t _regIndex, const void* _val, uint32_t _numBytes)
		{
			if (_flags & kUniformFragmentBit)
			{
				bx::memCopy(&m_fsScratch[_regIndex], _val, _numBytes);
				m_fsChanges += _numBytes;
			}
			else
			{
				bx::memCopy(&m_vsScratch[_regIndex], _val, _numBytes);
				m_vsChanges += _numBytes;
			}
		}

		void setShaderUniform4f(uint8_t _flags, uint32_t _regIndex, const void* _val, uint32_t _numRegs)
		{
			setShaderUniform(_flags, _regIndex, _val, _numRegs * 16);
		}
		...
		void commit(UniformBuffer& _uniformBuffer)
		{
			...
			for (;;)
			{
				...
				case UniformType::Mat3|kUniformFragmentBit:
					 {
						 ...
						 for (uint32_t ii = 0, count = num; ii < count; ++ii,  loc += 3*16, value += 9)
						 {
							 ...
							 mtx.un.val[11] = 0.0f;
							 setShaderUniform(uint8_t(type), loc, &mtx.un.val[0], 3 * 16);
						 }
					}
					break;
				...
				{
					float* value = (float*)data;
					for (uint32_t ii = 0, count = num; ii < count; ++ii, loc += 16, value += 1)
					{
						float val[4];
						val[0] = value[0];
						val[1] = 0.0;
						val[2] = 0.0;
						val[3] = 0.0;
						setShaderUniform(uint8_t(type), loc, val, sizeof(float) * 4);
					}
					//setShaderUniform(uint8_t(type), loc, value, num * sizeof(float));
				}
					break;
				...
				{
					float* value = (float*)data;
					for (uint32_t ii = 0, count = num; ii < count; ++ii, loc += 16, value += 2)
					{
						float val[4];
						val[0] = value[0];
						val[1] = value[1];
					...
					//setShaderUniform(uint8_t(type), loc, value, num * sizeof(float) * 2);
				}
					break;
				...
				{
					float* value = (float*)data;
					for (uint32_t ii = 0, count = num; ii < count; ++ii, loc += 16, value += 3)
					{
						float val[4];
						val[0] = value[0];
						val[1] = value[1];
					...
					//setShaderUniform(uint8_t(type), loc, value, num * sizeof(float) * 3);
				}
					break;
				...
				{
					float* value = (float*)data;
					setShaderUniform(uint8_t(type), loc, value, num * sizeof(float) * 4);
				}
				break;
				...
				{
					float* value = (float*)data;
					setShaderUniform(uint8_t(type), loc, value, num * sizeof(float) * 16);
				}
				break;
				...
				{
					int* value = (int*)data;
					setShaderUniform(uint8_t(type), loc, value, num * sizeof(int));
				}
				break;
	...
	void ShaderD3D11::create(const Memory* _mem)
	{
		...
		if (0 < count)
		{
			for (uint32_t ii = 0; ii < count; ++ii)
			{
				...
				{
					const UniformRegInfo* info = s_renderD3D11->m_uniformReg.find(name, UniformType::Enum(type & ~kUniformMask));
	...
	void* TextureD3D11::create(const Memory* _mem, uint64_t _flags, uint8_t _skip)
	{
		...
		if (bimg::imageParse(imageContainer, _mem->data, _mem->size) )
		{
			...
			m_autoGenMipmap = ti.autoGenMaipmap;
			...
			case TextureCube:
				{
					...
					desc.MipLevels  = (m_autoGenMipmap ? 0 : ti.numMips);
					...
					if (imageContainer.m_cubeMap)
					{
						...
						if (1 < ti.numLayers)
						{
							...
							srvd.TextureCubeArray.MipLevels = (m_autoGenMipmap ? -1 : ti.numMips);
							srvd.TextureCubeArray.NumCubes  = ti.numLayers;
						}
						else
						{
							srvd.ViewDimension = D3D11_SRV_DIMENSION_TEXTURECUBE;
							srvd.TextureCube.MipLevels = (m_autoGenMipmap ? -1 : ti.numMips);
						}
					}
					else
					{
						...
						else
						{
							if (1 < ti.numLayers)
							{
								...
								srvd.Texture2DArray.MipLevels = (m_autoGenMipmap ? -1 : ti.numMips);
								srvd.Texture2DArray.ArraySize = ti.numLayers;
							}
							else
							{
								srvd.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
								srvd.Texture2D.MipLevels = (m_autoGenMipmap ? -1 : ti.numMips);
							}
						}
					}
	...
	void RendererContextD3D11::submit(Frame* _render, TextVideoMemBlitter& _textVideoMemBlitter)
	{
		if (m_lost)
		{
			return;
		...
		if (0 == (_render->m_debug&BGFX_DEBUG_IFH) )
		{
			...
			for (int32_t item = 0; item < numItems;)
			{
				...
				if (viewChanged)
				{
					...
					{
						clearQuad(viewState.m_rect, clr, _render->m_colorPalette);
