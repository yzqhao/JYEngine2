	...
	void ShaderD3D12::create(const Memory* _mem)
	{
		...
		if (0 < count)
		{
			for (uint32_t ii = 0; ii < count; ++ii)
			{
				...
				{
					const UniformRegInfo* info = s_renderD3D12->m_uniformReg.find(name, UniformType::Enum(type & ~kUniformMask));
...
// 							| (blockInfo.stencilBits > 0 ? D3D12_DSV_FLAG_READ_ONLY_STENCIL : D3D12_DSV_FLAG_NONE)
	...
	void RendererContextD3D12::submit(Frame* _render, TextVideoMemBlitter& _textVideoMemBlitter)
	{
		if (m_lost
