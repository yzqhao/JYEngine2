			...
			for (uint32_t ii = 0; ii < count; ++ii)
			{
				...
				if (UINT16_MAX != regIndex)
				{
					...
					else if (UniformType::Sampler == (~kUniformMask & type) )
					{
						...
						const UniformRegInfo* info = s_renderVK->m_uniformReg.find(name, UniformType::Enum(type & ~kUniformMask));
...
#	error "Figure out KHR surface..."
	...
	void RendererContextVK::submit(Frame* _render, TextVideoMemBlitter& _textVideoMemBlitter)
	{
