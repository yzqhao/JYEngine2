...
namespace bgfx
{
	...
	/// Renderer device feature level.
	/// 
	/// @attention C99 equivalent is `bgfx_device_feature_level'
	///
	struct DeviceFeatureLevel
	{
		enum Enum
		{
			NONE,
	...
	struct Init
	{
		...
		/// Used to get feature level of current device.
		DeviceFeatureLevel::Enum featurelevel;
	...
	struct Encoder
	{
		/// Sets current thread viewID. 
		/// It's used for keep current thread render state
		///
	...
	/// Returns current device feature level.
	/// 
	...
	/// Create frame buffer for ref handle
	FrameBufferHandle createFrameBuffer(
		int32_t refHandle
		, uint16_t _width
		, uint16_t _height
